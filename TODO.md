...
- [x] **Honor `Enable`.** `INotificationFactory.All()` currently matches on settings type only; disabled connections can still drive deletes.
...
- [x] **Never delete a release that still has files** unless using Lidarr’s file-aware delete path. When skip is off, do not half-delete tracks then `DeleteMany` the release.
- [x] **Safer default when no allowed release remains.** Default `DeleteFiltered` can wipe every `AlbumRelease` for an album (including during incomplete refresh). Prefer keep-last-resort as default, or refuse to delete the last remaining copies.
- [x] **Empty media types must not silently become Vinyl, Cassette.** Validate the raw string; empty should be invalid or disable filtering. `ParseMediaTypes("")` currently always succeeds.
